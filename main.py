...
class Main(star.Star):
    ...
    async def writeback_shore_turn(
        ...
        payload = {
            ...
            "actor_account_uid": identity.actor_account_uid,
            "actor_person_uid": identity.actor_person_uid,
            "subject_person_uid": identity.subject_person_uid,
            "source_platform": identity.source_platform,
            "source": "astrbot",
            "scope_hint": identity.scope_hint,
            "domain": identity.domain_payload(),
            "observation_at": identity.observation_at,
            "alias_hints": identity.alias_hints_payload(),
            "messages": [
    ...
    async def shore_status(self, event: AstrMessageEvent):
        ...
        lines = [
            ...
            f"actor_person_uid: {identity.actor_person_uid}",
            f"domain: {identity.domain_payload().get('kind')}:{identity.domain_payload().get('key')}",
        ]
        yield event.plain_result("\n".join(lines)).stop_event()
    ...
    async def shore_remember(self, event: AstrMessageEvent, content: GreedyStr = ""):
        ...
        payload = {
            ...
            "content": memory_text,
            "domain": identity.domain_payload(scope=scope),
            "observation_at": identity.observation_at,
    ...
    async def _perform_recall(
        ...
        *,
        ...
        payload = {
            ...
            "actor_account_uid": identity.actor_account_uid,
            "actor_person_uid": identity.actor_person_uid,
            "focal_person_uids": [identity.actor_person_uid],
            "source_platform": identity.source_platform,
            "domain": identity.domain_payload(),
            "observation_at": identity.observation_at,
            "query": query,
